"""
Run V4 analysis and generate all outputs.
"""
import numpy as np
import pandas as pd
...
from analysis.core.io import PSAData, StrategyConfig  # noqa: E402
from analysis.core.log_utils import setup_script_logging, AnalysisContext  # noqa: E402
...
def create_v4_config():
    """Create V4 strategy configuration."""
    return StrategyConfig(
        base='UC',
        perspectives=['healthcare', 'societal'],
        strategies=[
            'UC', 'ECT', 'KA-ECT', 'IV-KA', 'IN-EKA',
            'PO-PSI', 'PO-KA', 'rTMS', 'UC+Li', 'UC+AA'
        ],
        prices={s: 0 for s in [
            'UC', 'ECT', 'KA-ECT', 'IV-KA', 'IN-EKA',
            'PO-PSI', 'PO-KA', 'rTMS', 'UC+Li', 'UC+AA'
...
def run_cea_analysis(psa: PSAData, output_dir: Path, figures_dir: Path, logger: logging.Logger):
    ...
    with AnalysisContext(logger, "cost_effectiveness_analysis"):
        ...
        # CE Plane
        try:
            cea_plots.plot_ce_plane(costs_wide, effects_wide, 'UC', figures_dir / 'ce_plane.png', 50000)
...
def run_voi_analysis(psa: PSAData, output_dir: Path, figures_dir: Path, logger: logging.Logger):
    ...
    with AnalysisContext(logger, "value_of_information_analysis"):
        ...
        # Generate parameter names for all strategies
        parameter_names = []
        strategy_params = {
            'IV-KA': 'iv_ketamine',
            'ECT': 'ect',
        ...
        for strategy, param_base in strategy_params.items():
            if strategy in psa.strategies or strategy.replace(' ', '_').lower() in [s.replace(' ', '_').lower() for s in psa.strategies]:
                parameter_names.extend([f'cost_{param_base}', f'effect_{param_base}'])

        # Remove duplicates (e.g., usual_care might appear twice)
...
def run_dcea_analysis(psa: PSAData, output_dir: Path, figures_dir: Path, logger: logging.Logger):
    ...
    with AnalysisContext(logger, "distributional_cost_effectiveness_analysis"):
        logger.info("=" * 60)
        logger.info("Running Distributional Cost-Effectiveness Analysis")
        logger.info("=" * 60)
...
def main():
    """Run complete V4 analysis."""
    # Set up logging
    logger = setup_script_logging("run_v4_analysis")

    logger.info("=" * 60)
    logger.info("V4 Analysis Pipeline")
    logger.info("=" * 60)
